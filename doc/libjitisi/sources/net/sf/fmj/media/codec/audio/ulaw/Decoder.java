...
public class Decoder extends AbstractCodec {
    ...
    public Format[] getSupportedOutputFormats(Format input) {
        ...
        if (input instanceof AudioFormat) {
            ...
            if (inputCast.getEncoding().equals(AudioFormat.ULAW) && ((inputCast.getSampleSizeInBits() == 8 || inputCast.getSampleSizeInBits() == -1) && ((inputCast.getChannels() == 1 || inputCast.getChannels() == -1) && ((inputCast.getSigned() == 1 || inputCast.getSigned() == -1) && ((inputCast.getFrameSizeInBits() == 8 || inputCast.getFrameSizeInBits() == -1) && (inputCast.getDataType() == null || inputCast.getDataType() == Format.byteArray)))))) {
                int endian = inputCast.getEndian();
                if (inputCast.getSampleSizeInBits() == 8) {
                    endian = 0;
                }
                AudioFormat result = new AudioFormat(AudioFormat.LINEAR, inputCast.getSampleRate(), 16, 1, endian, 1, 16, inputCast.getFrameRate(), Format.byteArray);
