...
import org.jitsi.bouncycastle.asn1.cms.RecipientKeyIdentifier;
...
import org.jitsi.bouncycastle.cms.KeyAgreeRecipientInfoGenerator;
...
public class JceKeyAgreeRecipientInfoGenerator extends KeyAgreeRecipientInfoGenerator {
    ...
    public ASN1Sequence generateRecipientEncryptedKeys(AlgorithmIdentifier algorithmIdentifier, AlgorithmIdentifier algorithmIdentifier2, GenericKey genericKey) throws CMSException {
        ...
        if (algorithm.getId().equals(CMSEnvelopedGenerator.ECMQV_SHA1KDF)) {
            mQVPrivateKeySpec = new MQVPrivateKeySpec(privateKey, this.ephemeralKP.getPrivate(), this.ephemeralKP.getPublic());
        } else {
            Object mQVPrivateKeySpec2 = privateKey;
        }
        ASN1EncodableVector aSN1EncodableVector = new ASN1EncodableVector();
        int i = 0;
        while (true) {
            int i2 = i;
            if (i2 == this.recipientIDs.size()) {
                return new DERSequence(aSN1EncodableVector);
            }
            Key key = (PublicKey) this.recipientKeys.get(i2);
            ...
            try {
                ...
                createKeyAgreement.init(mQVPrivateKeySpec2, this.random);
