...
import org.jitsi.bouncycastle.asn1.x509.DigestInfo;
import org.jitsi.bouncycastle.cms.jcajce.JcaSignerInfoVerifierBuilder;
import org.jitsi.bouncycastle.cms.jcajce.JcaSimpleSignerInfoVerifierBuilder;
...
public class SignerInformation {
    ...
    private boolean doVerify(PublicKey publicKey, Provider provider) throws CMSException, NoSuchAlgorithmException {
        ...
        if (provider != null) {
            try {
                build = !provider.getName().equalsIgnoreCase("BC") ? new JcaSignerInfoVerifierBuilder(new JcaDigestCalculatorProviderBuilder().build()).setProvider(provider).build(publicKey) : new JcaSimpleSignerInfoVerifierBuilder().setProvider(provider).build(publicKey);
    ...
    private boolean doVerify(SignerInformationVerifier signerInformationVerifier) throws CMSException {
        ...
        try {
            ...
            throw new CMSException("can't create content verifier: " + e32.getMessage(), e32);
        }
    }
