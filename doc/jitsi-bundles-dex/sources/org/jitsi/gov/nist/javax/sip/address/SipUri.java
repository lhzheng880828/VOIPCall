...
public class SipUri extends GenericURI implements SipURI, SipURIExt {
    ...
    public boolean equals(Object that) {
        ...
        if (getUser() != null && !UriDecoder.decode(getUser()).equals(UriDecoder.decode(b.getUser()))) {
            return false;
        }
        if (getUserPassword() != null && !UriDecoder.decode(getUserPassword()).equals(UriDecoder.decode(b.getUserPassword()))) {
            return false;
        }
        if (((getHost() == null ? 1 : 0) ^ (b.getHost() == null ? 1 : 0)) != 0) {
            return false;
        }
        if (getHost() != null && !getHost().equalsIgnoreCase(b.getHost())) {
            return false;
        }
        ...
        if (getHeaderNames().hasNext() && b.getHeaderNames().hasNext()) {
            try {
                ...
                while (i.hasNext()) {
                    ...
                    if (h1 != null || h2 != null) {
                        try {
                            if (!headerFactory.createHeader(hname, UriDecoder.decode(h1)).equals(headerFactory.createHeader(hname, UriDecoder.decode(h2)))) {
                                return false;
                            }
    ...
    public String getUserAtHost() {
        ...
        if (user.equals("")) {
            ...
            s = new StringBuilder(user).append(Separators.AT);
        }
    ...
    public void setTransportParam(String transport) throws ParseException {
        if (transport == null) {
            ...
            this.uriParms.set(new NameValue("transport", transport.toLowerCase()));
        } else {
