...
public class ChatStateManager {
    ...
    private class IncomingMessageInterceptor implements ChatManagerListener, MessageListener {
        ...
        public void processMessage(Chat chat, Message message) {
            ...
            if (extension != null) {
                try {
                    ChatStateManager.this.fireNewChatState(chat, ChatState.valueOf(extension.getElementName()), message);
                } catch (Exception e) {
                }
            }
        }
    ...
    public static ChatStateManager getInstance(Connection connection) {
        ...
        synchronized (managers) {
            WeakReference<ChatStateManager> ref = (WeakReference) managers.get(connection);
            if (ref == null) {
                manager = new ChatStateManager(connection);
                manager.init();
                managers.put(connection, new WeakReference(manager));
            } else {
    ...
    /* access modifiers changed from: private */
    public void fireNewChatState(Chat chat, ChatState state, Message message) {
