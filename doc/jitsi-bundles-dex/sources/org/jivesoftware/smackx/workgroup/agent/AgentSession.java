...
public class AgentSession {
    ...
    private void fireOfferRequestEvent(OfferRequestPacket requestPacket) {
        Offer offer = new Offer(this.connection, this, requestPacket.getUserID(), requestPacket.getUserJID(), getWorkgroupJID(), new Date(new Date().getTime() + ((long) (requestPacket.getTimeout() * 1000))), requestPacket.getSessionID(), requestPacket.getMetaData(), requestPacket.getContent());
        synchronized (this.offerListeners) {
    ...
    public void setNote(String sessionID, String note) throws XMPPException {
        note = StringUtils.escapeForXML(ChatNotes.replace(note, Separators.RETURN, "\\n"));
