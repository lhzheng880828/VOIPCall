...
public class ChatSettings extends IQ {
        ...
        private ChatSetting parseChatSetting(XmlPullParser parser) throws Exception {
            ...
            while (!done) {
                ...
                if (eventType == 2 && "key".equals(parser.getName())) {
                    key = parser.nextText();
                } else if (eventType == 2 && ParameterPacketExtension.VALUE_ATTR_NAME.equals(parser.getName())) {
    ...
    public String getChildElementXML() {
        ...
        buf.append('\"');
        if (this.key != null) {
            buf.append(" key=\"" + this.key + Separators.DOUBLE_QUOTE);
        }
        if (this.type != -1) {
            buf.append(" type=\"" + this.type + Separators.DOUBLE_QUOTE);
        }
