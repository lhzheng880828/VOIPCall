...
public class Bookmarks implements PrivateData {
    ...
    public String toXML() {
        ...
        for (BookmarkedURL urlStorage : getBookmarkedURLS()) {
            if (!urlStorage.isShared()) {
                buf.append("<url name=\"").append(urlStorage.getName()).append("\" url=\"").append(urlStorage.getURL()).append(Separators.DOUBLE_QUOTE);
                if (urlStorage.isRss()) {
                    buf.append(" rss=\"").append(true).append(Separators.DOUBLE_QUOTE);
                }
                buf.append(" />");
            }
        }
    ...
    public static BookmarkedConference getConferenceStorage(XmlPullParser parser) throws Exception {
        ...
        while (!done) {
            int eventType = parser.next();
            if (eventType == 2 && Nick.ELEMENT_NAME.equals(parser.getName())) {
                conf.setNickname(parser.nextText());
            } else if (eventType == 2 && ParameterNames.PASSWORD.equals(parser.getName())) {
