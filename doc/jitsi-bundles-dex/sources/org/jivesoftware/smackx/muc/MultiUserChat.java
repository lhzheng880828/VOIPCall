...
public class MultiUserChat {
    ...
    public synchronized void create(String nickname) throws XMPPException {
        if (nickname != null) {
            if (!nickname.equals("")) {
                ...
                joinPresence.setTo(this.room + Separators.SLASH + nickname);
                ...
                PacketCollector response = this.connection.createPacketCollector(new AndFilter(new FromMatchesFilter(this.room + Separators.SLASH + nickname), new PacketTypeFilter(Presence.class)));
                ...
                if (presence == null) {
                    ...
                    if (mucUser == null || mucUser.getStatus() == null || !("201".equals(mucUser.getStatus().getCode()) || "210".equals(mucUser.getStatus().getCode()) || "100".equals(mucUser.getStatus().getCode()) || "110".equals(mucUser.getStatus().getCode()))) {
                        leave();
    ...
    public synchronized void join(String nickname, String password, DiscussionHistory history, long timeout) throws XMPPException {
        if (nickname != null) {
            if (!nickname.equals("")) {
                ...
                try {
                    response = this.connection.createPacketCollector(new AndFilter(new FromMatchesFilter(this.room + Separators.SLASH + nickname), new PacketTypeFilter(Presence.class)));
                    this.connection.sendPacket(joinPresence);
    ...
    public synchronized void leave() {
        if (this.joined) {
            ...
            leavePresence.setTo(this.room + Separators.SLASH + this.nickname);
            for (PacketInterceptor packetInterceptor : this.presenceInterceptors) {
    ...
    public void changeAvailabilityStatus(String status, Mode mode) {
        if (this.nickname == null || this.nickname.equals("")) {
            ...
            joinPresence.setTo(this.room + Separators.SLASH + this.nickname);
            for (PacketInterceptor packetInterceptor : this.presenceInterceptors) {
                packetInterceptor.interceptPacket(joinPresence);
            }
            this.connection.sendPacket(joinPresence);
    ...
    private void init() {
        ...
        PacketMultiplexListener packetMultiplexor = new PacketMultiplexListener(this.messageCollector, new PacketListener() {
            public void processPacket(Packet packet) {
                ...
                String myRoomJID = MultiUserChat.this.room + Separators.SLASH + MultiUserChat.this.nickname;
