...
public class ConsoleDebugger implements SmackDebugger {
    ...
    private void createDebug() {
        ...
        this.connListener = new ConnectionListener() {
            public void connectionClosed() {
                System.out.println(ConsoleDebugger.this.dateFormatter.format(new Date()) + " Connection closed (" + ConsoleDebugger.this.connection.hashCode() + Separators.RPAREN);
            }

            public void connectionClosedOnError(Exception e) {
                System.out.println(ConsoleDebugger.this.dateFormatter.format(new Date()) + " Connection closed due to an exception (" + ConsoleDebugger.this.connection.hashCode() + Separators.RPAREN);
                e.printStackTrace();
            }

            public void reconnectionFailed(Exception e) {
                System.out.println(ConsoleDebugger.this.dateFormatter.format(new Date()) + " Reconnection failed due to an exception (" + ConsoleDebugger.this.connection.hashCode() + Separators.RPAREN);
                e.printStackTrace();
            }

            public void reconnectionSuccessful() {
                System.out.println(ConsoleDebugger.this.dateFormatter.format(new Date()) + " Connection reconnected (" + ConsoleDebugger.this.connection.hashCode() + Separators.RPAREN);
            }
    ...
    public void userHasLogged(String user) {
        System.out.println(("User logged (" + this.connection.hashCode() + "): " + ("".equals(StringUtils.parseName(user)) ? "" : StringUtils.parseBareAddress(user)) + Separators.AT + this.connection.getServiceName() + Separators.COLON + this.connection.getPort()) + Separators.SLASH + StringUtils.parseResource(user));
