...
class NonSASLAuthentication implements UserAuthentication {
    ...
    public String authenticate(String username, String resource, CallbackHandler cbh) throws XMPPException {
        try {
            cbh.handle(new Callback[]{new PasswordCallback("Password: ", false)});
            return authenticate(username, String.valueOf(new PasswordCallback("Password: ", false).getPassword()), resource);
        } catch (Exception e) {
    ...
    public String authenticateAnonymously() throws XMPPException {
        ...
        if (response == null) {
            ...
            return this.connection.getServiceName() + Separators.SLASH + ((Authentication) response).getResource();
        }
    }
}
